...
def solve(
    ...
    for i, board in enumerate(boards):
        ...
        logger.info(
            f"Completed all trees for board {board} ({i + 1} / {len(boards) + 1})."
        )
...
def solve_flop_lines(
    ...
    for size in bet_sizes:
        ...
        results = solver.calc_results()
        utils.write_results(
            datapath, strategy=f"{size}", results=results, position=position
        )
        solver.exit()
...
if __name__ == "__main__":
    ...
    solve(
        ...
        boards=utils.load_boards(
            "C:\PioSOLVER\preflop_subsets\PioSOLVER_2016_16_flops.txt"
        ),
        rootdir=ROOTDIR,
